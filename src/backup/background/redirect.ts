export function setupRequestInterceptor({ webRequest, handleLoginRedirectedBack, memexCloudOrigin }) {
...
export function makeRequestHandler({ handleLoginRedirectedBack }) {
    return ({ url, tabId }) => {
        if (tabId === -1) { // Came from bg script, prevent infinite loop of DOOM!
            return {}
        }
