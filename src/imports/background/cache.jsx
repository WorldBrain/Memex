...
export default class ImportCache {
    ...
    static async _getChunk(chunkKey, def = ImportCache.DEF_CHUNK) {
        const storage = await chrome.storage.local.get({ [chunkKey]: def })
    ...
    async flagItemAsError(itemKey, item) {
        ...
        if (!this.errStorageKeyStack.length) {
            ...
            errChunkKey =
                this.errStorageKeyStack[this.errStorageKeyStack.length - 1]
        }

        let { [errChunkKey]: existingChunk } = await chrome.storage.local.get({
            [errChunkKey]: {},
        })
