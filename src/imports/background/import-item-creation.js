...
async function checkWithExistingDocs() {
    ...
    return item => existingIds.has(`${pageKeyPrefix}${item.encodedUrl}`)
}
...
async function initFilterItemsByUrl() {
    ...
    const alreadyExists = await checkWithExistingDocs()
    ...
    return items =>
        items.reduce((accItems, currItem) => {
            ...
            try {
                ...
                const encodedUrl = encodeUrl(currItem.url)
                const augItem = { ...currItem, encodedUrl }

                // Filter out items already existing in DB
                return alreadyExists(augItem)
                    ? accItems
                    : [...accItems, augItem]
            } catch (error) {
...
export default async function createImportItems(allowTypes = DEF_ALLOW) {
    ...
    const historyItems = filterItemsByUrl(
        allowTypes[IMPORT_TYPE.HISTORY] ? await getHistoryItems() : [],
    )
    const bookmarkItems = filterItemsByUrl(
        allowTypes[IMPORT_TYPE.BOOKMARK] ? await getBookmarkItems() : [],
    )
    const oldExtItems = filterItemsByUrl(
        allowTypes[IMPORT_TYPE.OLD] ? await getOldExtItems() : [],
    )
