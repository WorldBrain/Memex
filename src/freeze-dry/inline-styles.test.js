...
describe('inlineStyles', () => {
    ...
    test('should return <style> tag with the fetched stylesheet', async () => {
        ...
        expect(doc.querySelector('style').innerHTML).toBe(`div{background-image: url(${imageDataUri});}`)
    })
    ...
    test('should convert the url in <style> to dataUris', async () => {
        ...
        expect(doc.querySelector('style').innerHTML).toBe(`div{background-image: url(${styleSheetAsDataUri});}`)
        spy.mockRestore()
    })
    ...
    test('should convert the url in "style" to dataUris', async () => {
        ...
        await inlineStyles({rootElement: doc.documentElement, docUrl})
        expect(spy).toHaveBeenCalled()
        expect(doc.querySelector('div').getAttribute('style')).toBe(`background-image: url(${imageDataUri});`)
        spy.mockRestore()
    })
})
