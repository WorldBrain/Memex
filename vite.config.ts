...
import dotenv from 'dotenv' // Load dotenv for custom env files

// Load custom env file (mimics build/env.js logic)
const mode = process.env.NODE_ENV || 'development'
const envPath =
    mode === 'development'
        ? resolve(__dirname, './private/.env.development')
        : resolve(__dirname, './private/.env.production')
console.log('Loading env from:', envPath) // Debug log
dotenv.config({ path: envPath })

// Optional: Fallback if file missing or vars not loaded
if (!process.env.FIREBASE_MEMEX_PROJECT_ID) {
    console.warn(
        'Warning: FIREBASE_MEMEX_PROJECT_ID not found – check your .env file!',
    )
}
...
export default defineConfig(async ({ command, mode: viteMode }) => {
    ...
    const define = {
        ...
        'process.env.FIREBASE_MEMEX_MESSAGING_SENDER_ID': JSON.stringify(
            process.env.VITE_FIREBASE_MEMEX_MESSAGING_SENDER_ID ||
                process.env.FIREBASE_MEMEX_MESSAGING_SENDER_ID,
        ),
        'process.env.FIREBASE_MEMEX_APP_ID': JSON.stringify(
            process.env.VITE_FIREBASE_MEMEX_APP_ID ||
                process.env.FIREBASE_MEMEX_APP_ID,
        ),
        'process.env.FIREBASE_MEMEX_MEASUREMENT_ID': JSON.stringify(
            process.env.VITE_FIREBASE_MEMEX_MEASUREMENT_ID ||
                process.env.FIREBASE_MEMEX_MEASUREMENT_ID,
        ),
        'process.env.FIREBASE_MEMEX_STORAGE_BUCKET': JSON.stringify(
            process.env.VITE_FIREBASE_MEMEX_STORAGE_BUCKET ||
                process.env.FIREBASE_MEMEX_STORAGE_BUCKET,
        ),
        'process.env.COUNTLY_APP_KEY': JSON.stringify(
            process.env.VITE_COUNTLY_APP_KEY || process.env.COUNTLY_APP_KEY,
        ),
        'process.env.COUNTLY_SERVER_URL': JSON.stringify(
            process.env.VITE_COUNTLY_SERVER_URL ||
                process.env.COUNTLY_SERVER_URL,
        ),
        'process.env.DEV_ANALYTICS': JSON.stringify(
            process.env.VITE_DEV_ANALYTICS || process.env.DEV_ANALYTICS,
        ),
        'process.env.FCM_VAPID_KEY': JSON.stringify(
            process.env.VITE_FCM_VAPID_KEY || process.env.FCM_VAPID_KEY,
        ),
        ...
        'import.meta.env.VITE_FIREBASE_MEMEX_MESSAGING_SENDER_ID':
            JSON.stringify(process.env.VITE_FIREBASE_MEMEX_MESSAGING_SENDER_ID),
